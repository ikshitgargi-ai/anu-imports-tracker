...
# OR the repo-root seeded demo DB. app.py reads DB_DIR (NOT DB_PATH) — the old
# DB_PATH env var here never worked, and this module's DELETE FROM stores /
# sod_inventory used to run against the repo-root SQLite file.
os.environ.pop('DATABASE_URL', None)
_TMP = tempfile.mkdtemp(prefix='anu_imports_sprint0_test_')
...
class TestRepCarryingFiltersStatusL:
    def test_rep_carrying_only_counts_listed_stores(self, app_module, client):
        ...
        # First tracked SKU
        first_sku = list(m.SOD_TRACKED_SKUS.keys())[0]  # e.g. '0045378'
...
class TestFreshnessFromSnapshotDate:
    ...
    def test_healthz_endpoint_consistent(self, app_module, client):
        ...
        Plain /healthz is a liveness probe and deliberately returns 200 even
